      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="space-y-2">
          ...
          <textarea
            ...
            placeholder="e.g., A best man speech for my brother who loves fishing, or a graduation speech about resilience..."
            className="w-full h-32 p-4 text-base text-black bg-gray-50 dark:bg-gray-700 dark:text-white dark:placeholder-gray-400 border border-gray-300 dark:border-gray-600 rounded-xl focus:ring-2 focus:ring-indigo-500 focus:border-transparent resize-none transition-all"
          />
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="space-y-2">
            ...
            <select
              value={tone}
              onChange={(e) => setTone(e.target.value as SpeechTone)}
              className="w-full p-2.5 bg-gray-50 dark:bg-gray-700 dark:text-white border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none transition-colors"
            >
              {Object.values(SpeechTone).map((t) => (
          ...
          <div className="space-y-2">
            ...
            <select
              value={length}
              onChange={(e) => setLength(e.target.value as SpeechLength)}
              className="w-full p-2.5 bg-gray-50 dark:bg-gray-700 dark:text-white border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none transition-colors"
            >
              {Object.values(SpeechLength).map((l) => (
           ...
           <div className="space-y-2">
            ...
            <input
              ...
              className="w-full p-2.5 bg-gray-50 dark:bg-gray-700 dark:text-white dark:placeholder-gray-400 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none transition-colors"
            />
          </div>
        ...
        <button
          type="submit"
          disabled={isLoading}
          className={`w-full py-4 px-6 rounded-xl text-white font-medium text-lg flex items-center justify-center gap-2 transition-all transform active:scale-[0.98] ${
            isLoading 
              ? 'bg-gray-400 dark:bg-gray-600 cursor-not-allowed' 
              : 'bg-indigo-600 hover:bg-indigo-700 dark:bg-indigo-600 dark:hover:bg-indigo-500 shadow-lg hover:shadow-indigo-500/30'
          }`}
        >
          {isLoading ? (
            <>
